...
Copernicus Workflow Integration
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

For data-sparse regions or synthetic calibration tests, the framework can generate observations using **Copernicus land cover data** and export coefficients. This estimates nutrient loads based on land use within each catchment.

**Enabling Copernicus Workflow**

In your calibration configuration file:

.. code-block:: python

    # Select Copernicus as observation data source
    observation_data_source = "copernicus"

    # Configure Copernicus workflow
    copernicus_config = {
        # River network shapefile
        "river_network_shapefile": "/path/to/river_network.shp",
        "reach_id_column": "seg_id",

        # Copernicus land cover data (GeoTIFF)
        "land_cover_path": "/path/to/copernicus_land_cover.tif",

        # Optional climate data for dynamic exports
...
**When to Use Copernicus Workflow**

* Data-sparse regions where GRQA has limited coverage
* Sensitivity analysis with controlled observation uncertainty
* Testing calibration workflows before using real observations
* Estimating background loads from land use
...
.. code-block:: bash
    ...
    # Normal run with Copernicus generation
    python my_calibration.py
