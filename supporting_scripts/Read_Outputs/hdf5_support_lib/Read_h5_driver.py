...
def create_openwq_ss_json_from_loads(
        ...
        hru_id_column: str,
        openwq_h5_output_path: str,
        metadata_comment: str = "Nutrient loading from land use/land cover",
        metadata_source: str = "Copernicus LULC analysis"
) -> None:
    """
    ...
    openwq_h5_output_path : str
        Path to OpenWQ HDF5 output file containing xyz_elements_source and mapping_key data
    ...
    # Load HDF5 mapping
    print(f"Loading spatial mapping from: {openwq_h5_output_path}")
    with h5py.File(openwq_h5_output_path, 'r') as h5f:
        # Load xyz_elements_source
        if 'xyz_elements_source' not in h5f:
            raise ValueError(f"'xyz_elements_source' not found in HDF5 file. Available: {list(h5f.keys())}")

        xyz_elements = h5f['xyz_elements_source'][:]

        # Load mapping key (e.g., Seg_ID)
        if hru_id_column not in h5f:
            raise ValueError(
                f"'{hru_id_column}' not found in HDF5 file. Available: {list(h5f.keys())}\n"
                f"Make sure the mapping_key matches a variable in the HDF5 file."
            )

        mapping_ids = h5f[hru_id_column][:]

    print(f"  Loaded {len(xyz_elements)} spatial elements")
    print(f"  xyz_elements shape: {xyz_elements.shape}")
    print(f"  {hru_id_column} shape: {mapping_ids.shape}")

    # Create mapping dictionary: HRU_ID -> (ix, iy, iz)
    hru_to_xyz = {}
    for idx in range(len(mapping_ids)):
        hru_id = mapping_ids[idx]
    ...
    exclude_cols = [hru_id_column, 'Year']
    ...
    for year in sorted(pivot_df['Year'].unique()):
        ...
        for nutrient in nutrient_cols:
            ...
            nutrient_loads = year_data[[hru_id_column, nutrient]].copy()
...
def set_ss_from_copernicus_lulc_with_loads(
    ...
    openwq_h5_output_path : str
        Path to OpenWQ HDF5 output file with xyz_elements_source and mapping_key
    metadata_comment : str
    ...
    create_openwq_ss_json_from_loads(
        ...
        compartment_name=compartment_name,
        hru_id_column=hru_id_column,
        openwq_h5_output_path=openwq_h5_output_path,
