...
void OpenWQ_readjson::SetConfigInfo_output_what2print(
    ...
    for (unsigned int chemi = 0; chemi < num_chem2print; chemi++){
        ...
        chem_name2print = OpenWQ_wqconfig.CH->NativeFlex->chem_species_list[chemi];
        for (unsigned int chemlisti = 0; chemlisti < (OpenWQ_wqconfig.CH->NativeFlex->num_chem); chemlisti++){ 
            chem_namelist = OpenWQ_wqconfig.CH->NativeFlex->chem_species_list[chemlisti];
