...
void OpenWQ_readjson::SetConfigInfo_CHModule_PHREEQC(
    ...
    OpenWQ_wqconfig.phreeqcrm = std::unique_ptr<PhreeqcRM>(new PhreeqcRM(1, 1));
    OpenWQ_wqconfig.phreeqcrm->LoadDatabase(database_filename);
    OpenWQ_wqconfig.phreeqcrm->RunFile(true,true, true, PHREEQC_filename);

    // Get number of chemical species from BGC_json
    (OpenWQ_wqconfig.BGC_general_num_chem) = OpenWQ_wqconfig.phreeqcrm->FindComponents();

    component_names = OpenWQ_wqconfig.phreeqcrm->GetComponents();
