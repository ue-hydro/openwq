...
void OpenWQ_CH_model::phreeqc_run(
    ...
    nxyz = OpenWQ_wqconfig.phreeqcrm->GetGridCellCount();


    c.resize(nxyz * (OpenWQ_wqconfig.BGC_general_num_chem));
    volumes.resize(nxyz);
    temperatures.resize(nxyz);
    pressures.resize(nxyz);
    indx = 0;
    pressures = OpenWQ_wqconfig.phreeqcrm->GetPressure();
    temperatures = OpenWQ_wqconfig.phreeqcrm->GetTemperature();
    ...
    OpenWQ_wqconfig.phreeqcrm->SetPressure(pressures);
    OpenWQ_wqconfig.phreeqcrm->SetTemperature(temperatures);
    OpenWQ_wqconfig.phreeqcrm->SetConcentrations(c);
    OpenWQ_wqconfig.phreeqcrm->SetRepresentativeVolume(volumes);

    OpenWQ_wqconfig.phreeqcrm->SetTimeStep(OpenWQ_hostModelconfig.get_time_step());
    if (OpenWQ_hostModelconfig.get_time_step() != 0)    OpenWQ_wqconfig.phreeqcrm->RunCells();
    OpenWQ_wqconfig.phreeqcrm->GetConcentrations(c);
    for (unsigned int chemi=0;chemi<(OpenWQ_wqconfig.BGC_general_num_chem);chemi++){
        indx = 0;
        for (unsigned int icmp=0;icmp<OpenWQ_hostModelconfig.get_num_HydroComp();icmp++){
