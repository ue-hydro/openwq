...
void OpenWQ_CH_model::phreeqc_setup(
    ...
    OpenWQ_wqconfig.phreeqcrm = std::unique_ptr<PhreeqcRM>(new PhreeqcRM(nxyz, OpenWQ_wqconfig.get_num_threads_requested()));
    ...
    OpenWQ_wqconfig.phreeqcrm->SetComponentH2O(component_h2o);

    OpenWQ_wqconfig.phreeqcrm->SetFilePrefix("phreeqc");
    OpenWQ_wqconfig.phreeqcrm->OpenFiles();


    database = OpenWQ_json.BGC_module["DATABASE"];
    OpenWQ_wqconfig.phreeqcrm->LoadDatabase(database);
    PHREEQC_filename = OpenWQ_json.BGC_module["FILEPATH"];
    OpenWQ_wqconfig.phreeqcrm->RunFile(true,true, true, PHREEQC_filename);
    OpenWQ_wqconfig.phreeqcrm->FindComponents();
