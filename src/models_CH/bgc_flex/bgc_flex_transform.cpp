...
void OpenWQ_CH_model::bgc_flex_transform(
    ...
    try{
        ...
        for (unsigned int bgci=0;bgci<num_BGCcycles;bgci++){
            ...
            for (unsigned int index_j=0;index_j<OpenWQ_wqconfig.CH->NativeFlex->BGCexpressions_info.size();index_j++){
                BGCcycles_name_list = std::get<0>(OpenWQ_wqconfig.CH->NativeFlex->BGCexpressions_info.at(index_j)); // get BGC cycle from OpenWQ_wqconfig.CH->NativeFlex->BGCexpressions_info
            ...
            for (unsigned int transi=0;transi<num_transf;transi++){
                ...
                index_cons = std::get<3>(OpenWQ_wqconfig.CH->NativeFlex->BGCexpressions_info.at(transf_index[transi]));
                index_prod = std::get<4>(OpenWQ_wqconfig.CH->NativeFlex->BGCexpressions_info.at(transf_index[transi]));
                ...
                std::vector<unsigned int> index_chemtransf = 
                    std::get<5>(
                        OpenWQ_wqconfig.CH->NativeFlex->BGCexpressions_info.at(transf_index[transi]));
                ...
                for (unsigned int ix=0;ix<nx;ix++){
                    for (unsigned int iy=0;iy<ny;iy++){
                        for (unsigned int iz=0;iz<nz;iz++){                    
                            ...
                            OpenWQ_wqconfig.CH->NativeFlex->chemass_InTransfEq.clear();
                            // loop to get all the variables inside the expression
                            for (unsigned int chem=0;chem<index_chemtransf.size();chem++){
                                OpenWQ_wqconfig.CH->NativeFlex->chemass_InTransfEq.push_back(
                                    (*OpenWQ_vars.chemass)
                                    (icmp)
                            ...
                            transf_mass = std::fmax(
                                OpenWQ_wqconfig.CH->NativeFlex->BGCexpressions_eq[transf_index[transi]].value(),
