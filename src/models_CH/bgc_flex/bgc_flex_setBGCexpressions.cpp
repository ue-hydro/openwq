    ...
    for (unsigned int bgci=0;bgci<num_BGCcycles;bgci++){
        ...
        for (unsigned int transi=0;transi<num_transf;transi++){
            ...
            for (unsigned int i=0;i<index_transf.size();i++){
                OpenWQ_wqconfig.CH->NativeFlex->chemass_InTransfEq.push_back(0); // creating the vector
            }

            // Add vectors to table of symbols
            symbol_table.add_vector("openWQ_BGCnative_chemass_InTransfEq",OpenWQ_wqconfig.CH->NativeFlex->chemass_InTransfEq);
            ...
            OpenWQ_wqconfig.CH->NativeFlex->BGCexpressions_info.push_back(
                BGCTransfTuple_info(
                    BGCcycles_name,
                    Transf_name,
                    expression_string,
            ...
            OpenWQ_wqconfig.CH->NativeFlex->BGCexpressions_eq.push_back(expression);
