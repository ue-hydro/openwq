...
void OpenWQ_initiate::setIC_driver(
    ...
    for (unsigned int chemi=0;chemi<(OpenWQ_wqconfig.CH_model->NativeFlex->num_chem);chemi++){
        ...
        // Get chem name
        chemname = (OpenWQ_wqconfig.CH_model->NativeFlex->chem_species_list)[chemi]; // chemical name in BGC-json list
...
void OpenWQ_initiate::setIC_json(
    ...
    // Get chem name
    std::string chemname = (OpenWQ_wqconfig.CH_model->NativeFlex->chem_species_list)[chemi]; // chemical name in BGC-json list
